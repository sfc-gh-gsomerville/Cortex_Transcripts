-- Create Cursor_Demo database and V1 schema if they don't exist
CREATE DATABASE IF NOT EXISTS Cursor_Demo;
CREATE SCHEMA IF NOT EXISTS Cursor_Demo.V1;

-- Create agents table
CREATE OR REPLACE TABLE Cursor_Demo.V1.SUPPORT_AGENTS (
    AGENT_ID INT AUTOINCREMENT PRIMARY KEY,
    AGENT_NAME VARCHAR(100)
);

-- Insert agents
INSERT INTO Cursor_Demo.V1.SUPPORT_AGENTS (AGENT_NAME)
VALUES 
    ('Alice Johnson'),
    ('Bob Smith'),
    ('Charlie Brown'),
...
CREATE OR REPLACE TABLE Cursor_Demo.V1.CUSTOMERS (
    CUSTOMER_ID INT AUTOINCREMENT PRIMARY KEY,
    CUSTOMER_NAME VARCHAR(100)
);

-- Insert customers (we'll insert 100 unique customers)
INSERT INTO Cursor_Demo.V1.CUSTOMERS (CUSTOMER_NAME)
VALUES 
...
CREATE OR REPLACE TABLE Cursor_Demo.V1.HOME_MEDICAL_DEVICES (
...
INSERT INTO Cursor_Demo.V1.HOME_MEDICAL_DEVICES (
    DEVICE_NAME,
    CATEGORY,
    SUBCATEGORY,
...
CREATE OR REPLACE TABLE Cursor_Demo.V1.SUPPORT_CONVERSATIONS (
...
INSERT INTO Cursor_Demo.V1.SUPPORT_CONVERSATIONS (
    START_TIME,
    END_TIME,
    AGENT_ID,
    CUSTOMER_ID,
...
CREATE OR REPLACE PROCEDURE Cursor_Demo.V1.GENERATE_TRANSCRIPTS()
RETURNS VARCHAR
LANGUAGE SQL
AS
$$
DECLARE
    conv_cursor CURSOR FOR SELECT 
        ...
        SC.DEVICE_NAME,
        SC.COMMON_ISSUE
    FROM Cursor_Demo.V1.SUPPORT_CONVERSATIONS SC
...
BEGIN
    FOR record IN conv_cursor DO
        ...
        UPDATE Cursor_Demo.V1.SUPPORT_CONVERSATIONS 
...
CALL Cursor_Demo.V1.GENERATE_TRANSCRIPTS();
...
SELECT 
    ...
    START_TIME,
    END_TIME,
    (SELECT AGENT_NAME FROM Cursor_Demo.V1.SUPPORT_AGENTS WHERE AGENT_ID = sc.AGENT_ID) AS AGENT_NAME,
    (SELECT CUSTOMER_NAME FROM Cursor_Demo.V1.CUSTOMERS WHERE CUSTOMER_ID = sc.CUSTOMER_ID) AS CUSTOMER_NAME,
    DEVICE_NAME,
