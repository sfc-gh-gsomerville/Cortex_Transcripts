...
CREATE OR REPLACE TABLE Cursor_Demo.ANALYTICS.RAW_JSON_DATA_INITIAL (
    FILE_NAME VARCHAR,
...
COPY INTO "CURSOR_DEMO"."ANALYTICS"."RAW_JSON_DATA_INITIAL" 
FROM (SELECT 
    METADATA$FILENAME,
    CURRENT_TIMESTAMP(),
    $1::VARIANT
    FROM '@"CURSOR_DEMO"."DATA_PREP"."CALL_DATA_INITIAL"') 
...
AS
    ...
    FROM (SELECT 
        ...
        CURRENT_TIMESTAMP(),
        $1::VARIANT
        FROM '@"CURSOR_DEMO"."DATA_PREP"."CALL_DATA_NEW"') 
    FILE_FORMAT = '"CURSOR_DEMO"."ANALYTICS"."JSON_GZ_FORMAT"' 
    ON_ERROR=ABORT_STATEMENT 
 ;
...
CREATE OR REPLACE PROCEDURE Cursor_Demo.ANALYTICS.RUN_NEW_TRANSCRIPT_PIPELINE()
    RETURNS STRING
    LANGUAGE JAVASCRIPT
    EXECUTE AS CALLER
AS
$$
    try {
        ...
        var resume_stmt = snowflake.createStatement({
            sqlText: "ALTER TASK Cursor_Demo.ANALYTICS.LOAD_JSON_FILES_NEW RESUME;"
        });
        resume_stmt.execute();
        ...
        var suspend_stmt = snowflake.createStatement({
            sqlText: "ALTER TASK Cursor_Demo.ANALYTICS.LOAD_JSON_FILES_NEW SUSPEND;"
        });
...
CALL Cursor_Demo.ANALYTICS.RUN_NEW_TRANSCRIPT_PIPELINE();